...
class UdpPacket(Packet):
    ...
    def __init__(
        ...
        :param source_port: Source port field value, integer in range [0; 65535]. 0 by default
    ...
    def to_bytes(self):
        length = self.UDP_HEADER_LENGTH_BYTES + len(self.payload)
        ...
        struct.pack_into(self.UDP_HEADER_FORMAT, header_buffer, 0, *header_fields)

        # generate pseudo header using underlying IP packet
        pseudo_header = TransportLayerUtils.get_pseudo_header(self, length)
        # calculate checksum
        checksum_bytes = Utils.calc_checksum(pseudo_header + header_buffer + self.__payload)
        ...
        return TransportLayerUtils.validate_packet_length(bytes(header_buffer) + self.__payload)
    ...
    def from_bytes(packet_bytes: bytes):
        ...
        header_fields = struct.unpack(UdpPacket.UDP_HEADER_FORMAT, header_bytes)
    ...
    @property
    def length(self) -> int:
        return self.UDP_HEADER_LENGTH_BYTES + len(self.payload)
    ...
    def __str__(self) -> str:
        return f"UDP(dst_port={self.dest_port}, src_port={self.source_port}, length={self.length})"
