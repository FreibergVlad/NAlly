...
class IpUtils:
    """
    Stores constants related to IP protocol and methods for IP fields
    validation
    ...
    def validate_fragment_offset(fragment_offset: int):
        ...
        :raises: ValueError: if passed value doesn't satisfy bit length
            requirements
        """
        if fragment_offset.bit_length() > IpUtils\
                .IP_V4_MAX_FRAG_OFFSET_LENGTH_BITS:
            raise ValueError(f"Fragmentation offset should be "
                             f"{IpUtils.IP_V4_MAX_FRAG_OFFSET_LENGTH_BITS} "
                             f"bit length")
    ...
    def validate_or_gen_packet_id(packet_id: int = None):
        ...
        if packet_id.bit_length() > IpUtils.IP_V4_MAX_ID_LENGTH_BITS:
            raise ValueError(f"Packet identification should be "
                             f"{IpUtils.IP_V4_MAX_ID_LENGTH_BITS} "
                             f"bit length")
        return packet_id
    ...
    def validate_packet_length(total_length: int):
        """
        Validates Total Length IPv4 header field. Value should be in range
            [20, 65535]

        :param int total_length: total length of IP packet including header
            and payload
        :return: validated total_length value
        :raises: ValueError: if passed value isn't in required range
        """
        if total_length < IpUtils.IP_V4_MAX_HEADER_LENGTH_BYTES \
                or total_length > IpUtils.IP_V4_MAX_PACKET_LENGTH_BYTES:
            raise ValueError(f"Packet length should be <= "
                             f"{IpUtils.IP_V4_MAX_PACKET_LENGTH_BYTES} bytes")
        return total_length

    @staticmethod
    def gen_fragment_id() -> int:
        """
        Generates random 16 bits value which can be used as Identification
